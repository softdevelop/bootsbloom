...
class CsvImportTest extends CakeTestCase
{
    ...
    public function testImportCSVWithValidation()
    {
        $this->Content->Behaviors->load("Utils.CsvImport");
        $path = App::pluginpath("Utils");
        $this->Content->validate = array( "title" => array( "long" => array( "rule" => array( "minLength", 100 ) ) ) );
        $result = $this->Content->importCSV($path . "Test" . DS . "tmp" . DS . "test1.csv");
        $this->assertFalse($result);
        $errors = $this->Content->getImportErrors();
        $expected = array( array( "validation" => array( "title" => array( "long" ) ) ), array( "validation" => array( "title" => array( "long" ) ) ) );
    ...
    public function testImportCSVWithSomeErrors()
    {
        $this->Content->Behaviors->load("Utils.CsvImport");
        $path = App::pluginpath("Utils");
        $this->Content->validate = array( "type" => array( "list" => array( "rule" => array( "inList", array( "Article" ) ) ) ) );
        $result = $this->Content->importCSV($path . "Test" . DS . "tmp" . DS . "test1.csv", array(  ), true);
        $this->assertEqual($result, array( 0 ));
        $errors = $this->Content->getImportErrors();
        $expected = array( 1 => array( "validation" => array( "type" => array( "list" ) ) ) );
        $this->assertEqual($errors, $expected);
    }
