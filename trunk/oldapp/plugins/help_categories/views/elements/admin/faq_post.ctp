		...
		<table width="100%" align="center" border="0" cellspacing="0" cellpadding="3">
		   <tr class="ui-widget-header ui-corner-all" style="height:30px;">
			   <td align="left" class="padleft_15px" width="5%"><?php echo $form->checkbox('chkAll',array('id' => 'chkAll','name' => 'chkAll','value' => '','onclick' => 'checkAll()')); ?></td>
				...
				<td align="left" width="35%"><?php echo $paginator->sort('Title', 'post_title');?><span <?php if($sortKey=="FaqPost.post_title"){?> class="<?php echo $sortDir;?>" <?php }?>>&nbsp;</span></td>
				
				<td align="left" width="20%">Category Name</td>
				
				<td align="center" width="20%"><?php echo $paginator->sort('Created Date', 'created');?><span <?php if($sortKey=="FaqPost.created"){?> class="<?php echo $sortDir;?>" <?php }?>>&nbsp;</span></td>
			...
			<tr class="<?php echo $class;?>" id="row_<?php echo $records[$model]['id'];?>">
				...
				<td width="35%" valign="top">
					<?php echo ucwords(substr($records[$model]['post_title'],0,38)); ?>
					<div id="action_row_<?php echo $records[$model]['id'];?>" style="visibility:hidden;" class="action_row">
						<?php echo $this->Html->link('Edit',array('action' => 'admin_edit_faq_post',$records[$model]['id'],$parent_id),array('class'=>'vtip','title'=>'Edit Post')); ?> &nbsp;|&nbsp;
						<?php 
								if($records[$model]['active'] == '0'){
									
									echo $this->Html->link('Active',array('action' => 'admin_faq_post_status',$records[$model]['active'],$records[$model]['id']),array('class'=>'vtip','title'=>'Change category Status To Active'),
										'Are you sure you want to change Post status ?');
									
								}else{
									echo $this->Html->link('Inactive',array('action' => 'admin_faq_post_status',$records[$model]['active'],$records[$model]['id']),array('class'=>'vtip','title'=>'Change Category Status To Inactive'),
										'Are you sure you want to change Post status ?');
									
								}
						?>&nbsp;|&nbsp;		
						<?php echo $this->Html->link('Delete',array('action' => 'delete_faq_post',$records[$model]['id']),array('class'=>'vtip',	'title'=>'Delete Post'),'Are you sure you want to delete Post ?'); 
						?>
					</div>
				</td>
