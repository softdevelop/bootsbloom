...
$(document).ready(function() {
            ...
            <div class="floatleft padtop_20px">New Category</div>
               <div class="floatright padtop_6px">
                    <div class="floatleft padright_10px" style="font-size:10px;">
                        <?php 
						if($add_category[$model]['parent_id']!=0){
							echo $this->Html->link("Back To Categories",array("plugin"=>"help_categories","controller"=>"schools","action"=>"index",$add_category[$model]['parent_id']),array("class"=>'back_lnk'));
						}else{
							echo $this->Html->link("Back To Categories",array("plugin"=>"help_categories","controller"=>"schools","action"=>"index"),array("class"=>'back_lnk'));
						}
						?>
                    </div>
                    <div class="clear"></div>
	...
	<div id="tab1" class="tab_content">
		...
		<?php 	echo $this->Form->create($model,array('url'=>array('plugin'=>'help_categories','controller' => 'schools', 'action' => 'add_category',$parent_id,'eng'),"type"=>"file"));
                        if(isset($parent_id)) {
				echo $this->Form->hidden('parent_id',array('value'=>$parent_id));  
				echo $this->Form->hidden('section',array('value'=>'school'));  
			}
			?>
			...
			<tr class="odd_row">
					<td align='left' width="19%" valign="top" class="padding_left_40">Category Image<font color='red'>*</font></td>
					<td width="1%" valign="top">:</td>
					<td > <?php echo $form->file($model.".category_image", array("class" => "ui-widget-content ui-corner-all")); ?></td>
					<td width="30%" valign="middle">
					<?php  if(isset($this->validationErrors[$model]['category_image'])){ ?>
						<div  class="errormsg">
						  <div class="floatleft padright_10px padtop_4px"><img src="<?php echo WEBSITE_URL;?>img/error_icon.gif" width="10" height="10" alt="" /></div>
						  <div class="floatleft"><?php echo $this->validationErrors[$model]['category_image'];?></div>
						  <div class="clear"></div>
						  <div class="errtl"></div>
						  <div class="errtr"></div>
						  <div class="errbl"></div>
