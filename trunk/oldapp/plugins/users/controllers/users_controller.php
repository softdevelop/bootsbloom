...
class UsersController extends UsersAppController {
	...
	function fblogin() {
		...
		try
		{
			...
			if(!isset($user_profile['id'])) throw new Exception();

			if($this->Auth->user())
			{
				if($this->User->field('facebook_id') && $this->User->field('facebook_id') != $user_profile['id']) throw new Exception();
			...
			else
			{
				...
				if($existingUser)
				{
					if($existingUser['User']['facebook_id'] && $existingUser['User']['facebook_id'] != $user_profile['id']) throw new Exception();
        ...
        $this->set('title_for_layout',__('delete_account',true));
         if ($this->data) {
               if ($this->data['User']['password'] != $this->Session->read('Auth.User.password_token')) {
                ...
                //pr($user_projects);
                $this->Project->updateAll(array("is_cancelled" => 1), array('Project.user_id'=>$user_id,'Project.active'=>1,'Project.is_successful'=>0));
